...
### LiveKit `turn_handling` migration + dynamic endpointing + tighter defaults
`AgentSession` migrated off the flat deprecated kwargs (`min_endpointing_delay=`, `min_interruption_words=`, etc.) to the canonical `turn_handling` TypedDict — required before SDK 2.x. While moving, three default changes shipped: **(1)** endpointing mode now `"dynamic"`, so the SDK tracks per-session pause statistics via EMA (`alpha=0.9`) and slides the wait inside `[min_delay, max_delay]` per user. Zero added latency — only different wait duration; first 2-3 turns of a session still behave like fixed because the EMA hasn't seeded. **(2)** `min_endpointing_delay` 0.5 → 0.3 and `max_endpointing_delay` 5.0 → 2.0. With preemptive TTS already firing during the silence window, dropping the floor shaves ~200 ms perceived turn-end; capping the ceiling at 2 s removes the worst-case 5-second dead-air pause (which was the worst-feeling failure mode for Arabic, where the turn detector gives no semantic signal). **(3)** `min_interruption_words` 0 → 1, filtering coughs / mic bumps / lip smacks from triggering false interrupts. A 0.5-s "stop!" still interrupts because `min_duration` triggers first. Interruption mode kept on `"vad"` — `"adaptive"` requires LiveKit's hosted inference service and silently falls back to VAD on self-hosted anyway. `preemptive_tts: True` retained. See [agent/agent.py::AgentSession](../agent/agent.py), [agent/config.py::AgentSettings](../agent/config.py).

### Self-hosted noise cancellation via DeepFilterNet3 (off by default)
New optional FrameProcessor at [`agent/plugins/denoiser.py`](../agent/plugins/denoiser.py) wrapping DeepFilterNet3 (Apache-2.0). Self-hosted-friendly — no LiveKit Cloud dependency, no commercial license like `livekit-plugins-krisp` would require. Algorithmic latency ~10 ms (20 ms STFT window − 10 ms hop, `df_lookahead=0`); compute ~2 ms per 10 ms frame at server-CPU RTF 0.19 from the paper. Resamples 16↔48 kHz internally via `df.io.resample` (sinc_fast). Wired into `room_io.AudioInputOptions.noise_cancellation` when `AGENT_NOISE_CANCELLATION=true`; default off because installing `deepfilternet` pulls PyTorch (~600 MB image bloat). Caveats documented in the module docstring: per-call LSTM state reset means sub-frame boundary artifacts at 50-ms frame boundaries (inaudible for ASR); first-session model load ~50–200 ms; per-session FrameProcessor instance owns its own DF state. Considered and rejected: LiveKit Cloud BVC (`noise_cancellation.BVC()`) silently no-ops off Cloud; `livekit-plugins-krisp` needs a commercial Krisp SDK license; client-side `@livekit/krisp-noise-filter` works but is a demo-only change that doesn't cover other clients.
